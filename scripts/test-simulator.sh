...
if xcodebuild -project PhotoServer.xcodeproj -scheme PhotosIntegration -configuration Debug \
  -destination "platform=iOS Simulator,id=$device" -parallel-testing-enabled NO -derivedDataPath build/Simulator \
  -resultBundlePath build/PhotosIntegration.xcresult CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY='' \
  test >build/photos-ui.log 2>&1; then
  echo "Photos UI probe finished."
else
  echo "Photos UI probe did not pass; recorded as unverified, not as a failed IPA build."
  tail -80 build/photos-ui.log
fi
