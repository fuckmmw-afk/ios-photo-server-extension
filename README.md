...
Proxy остаётся на серверном `127.0.0.1:4981`. Для iPhone нужен SSH-клиент с local forwarding: локальный `localhost:4981` → серверный `127.0.0.1:4981`. Туннель должен продолжать работать, пока открыто Photos; некоторые iOS SSH-клиенты при переходе в фон приостанавливают соединение. На симуляторе туннель запускается на Mac: `ssh -N -L 4981:127.0.0.1:4981 USER@SERVER`.

В приложении по умолчанию `http://localhost:4981`. Разрешён HTTPS или HTTP исключительно на loopback. Общего ATS bypass нет. Публичная публикация proxy и раздача cookies не нужны. Единственная настройка — URL подключения, общая через App Group. На физическом iPhone без устойчивого туннеля end-to-end работа не гарантируется.
...
- При сохранении JPEG копируется; PNG один раз преобразуется в JPEG качества 1.0 согласно контракту базового `renderedContentURL`. Размер не уменьшается. Orientation/профиль берутся из результата; старые EXIF и геометрия оригинала не переносятся поверх нового изображения.
