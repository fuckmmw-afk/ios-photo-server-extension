...
Модели динамически читаются из авторизованного `GET /openai/v1/models` (`data[].id`). При первом подключении выбирается доступная Flash-модель, предпочитая не-lite вариант с наибольшим номером версии; пользователь может выбрать любую опубликованную модель в настройках. Если сохранённый выбор пропал из списка, приложение и Photos extension автоматически выбирают текущий подходящий вариант. Перед каждой обработкой extension заново загружает список. Название внутреннего image-generator не подтверждено; это не обещание вызова API-модели `gemini-3.1-flash-image`. Временный промпт находится в `Shared/Settings.swift` и сохраняет содержимое, лица и композицию; продуктовый промпт будет добавлен позже. Генеративная модель не гарантирует побитовое сохранение деталей.

## Подключение

Proxy слушает только `127.0.0.1:4981` на сервере. Для iPhone он доступен по HTTPS через gateway, который сохраняет `X-PhotoServer-Key`. Google-сессией управляет постоянный профиль Chrome на VPS, а не переменная `GEMINI_COOKIES`. Настройки проверяют живой статус Gemini; HTTP 503 показывается как временная недоступность, а не как успешное подключение. При «Требуется вход» нажмите «Открыть ссылку для входа» и вручную войдите в Google во временном браузере VPS. Одноразовую ссылку нужно открыть за минуту; если она истекла, нажмите кнопку снова. `localhost` на телефоне — это сам телефон, не VPS.
